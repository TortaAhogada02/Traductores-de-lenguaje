# Alumno: OMAR ALEJANDRO QUIROZ TRUJILLO
# Materia: TRADUCTORES DE LENGUAJE II
# Maestro: MICHEL EMANUEL LOPEZ FRANCO

from anytree import Node, RenderTree
from generacion import codigo
import graf_ as gr
import main_func as fun
...
class analizador:
    ...
    def analizadorsintactico(self, i, auxelimna2, divcad2):
        while True:
            for obj in pila:
                try:
                    gr.txtE2.insert(gr.END, obj.cad)
                except:
                    gr.txtE2.insert(gr.END, obj)
            gr.txtE2.insert(gr.END, "\n")
            if divcad2[i]=='print':
            ...
            columna = fun.buscar(divcad2[i])
            accion = matrizreglas[fila][columna.pos]
            accion= estado(str(accion), accion, accion, accion)
            if accion.estado == 0:
                gr.txtE3.insert(gr.END, 'Error' + '\n')
                break
            elif accion.estado > 0:
                i+=1
                pila.append(columna)
                pila.append(accion)
                gr.txtE3.insert(gr.END, 'd' + accion.cad + '\n')
            elif accion.estado  <0:
                if accion.estado == -1:
                    gr.txtE3.insert(gr.END, 'r0 acept')
                    break
                else:
                    for obj in lisreglas:
                        if accion.estado == (obj.aux +1) * -1:
                            gr.txtE3.insert(gr.END, 'r' + str(obj.aux) + ": " + obj.regla + '\n')
...
for objlex in listalexico:
    gr.tab.insert("", gr.END, text=objlex.cad, values=(objlex.tipo, objlex.pos))
...
if len(listaerrores)!=0:
    ...
    for pre, fill, node in RenderTree(root):
        gr.txtE4.insert(gr.END, "%s%s" % (pre, node.name) + '\n')

    for obj in listavariables:
        gr.tabS.insert("", gr.END, text=obj.data.cad, values=(obj.tipo.cad, obj.lv))
